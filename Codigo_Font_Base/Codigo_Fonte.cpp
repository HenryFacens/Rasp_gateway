...
//exemplo de mensagem, comeÄ‡ando em $ e terminando com \n. Os Bytes internos representam a mensagem
...
//! O nÃºmero de rÃ¡dios que deve ser inicializado. EstÃ¡ parametrizado no arquivo ini, seÃ§Ã£o start, campo initialize. O padrÃ£o Ã© 3
...
string basedir; //!diretorio primÃ¡rio comum aos arquivos.
...
//!VariÃ¡vel utilizada para compor as queries do banco de dados.
char q[150] = {0};
...
//! Separa os bytes da mensagem recebida em suas respectivas variÃ¡veis
...
//!Gerador de logs dos eventos de erro do sistema para auxÃ­lio na depuracao.
void log(string msg);

//!Substituicao de caracteres em um array.
char* replace_char(char* str, char find, char replace);

//!Checa se jÃ¡ hÃ¡ uma instancia rodando para nÃ£o executar 2x o programa.
...
//!Instancias dos rÃ¡dios
RH_RF95 rf95_u2(RF_CS_PIN, RF_IRQ_PIN);
RH_RF95 rf95_u3(RF_CS_PIN2, RF_IRQ_PIN2);
RH_RF95 rf95_u7(RF_CS_PIN3, RF_IRQ_PIN3);

//!Array dos objetos do rÃ¡dio, para fÃ¡cil manipulacao.
RH_RF95 rf95[3] = {rf95_u2,rf95_u3,rf95_u7};

//!Utilizado no loop para chavear a leitura dos rÃ¡dios.
unsigned char radioNumber = 0;

/*!Estrutura de servicos dedicados a cada rÃ¡dio. Esses valores sao passados atraves do arquivo /home/pi/AFMultiRadio.ini.
 * listener: rÃ¡dios que fazem escuta.
 * sender: rÃ¡dios que enviam mensagem
 * bridge: rÃ¡dios que fazem a transferencia dos arrays de dados das coletas de leitura para os demais concentradores.
 * starts: rÃ¡dios que devem ser inicializados com o programa, para placas de 1 a 3 rÃ¡dios. A ordem Ã© u2, u3 e u7
...
void db_query(const char *query, bool usedb){
    ...
    char *zErrMsg = 0;
    //Na declaracao na estrutura dbConf estÃ¡ > 0 (nao OK), entao tenta abrir se for a primeira vez...
    if (dbConf.rc){ //SQLITE_OK == 0 para ok
        //MAS se nao conseguir abrir, dÃ¡ mensagem de erro e tenta fechar mesmo assim.
...
/*! Funcao principal. Todo o programa e executado dentro dessa funcao. A primeira tarefa e validar o usuÃ¡rio; se
...
int main (int argc, char *argv[] ){
    ...
    /*O BCM faz acesso exclusivos que nao podem ser manipulados pelo usuÃ¡rio, portanto e necessÃ¡rio verificar quem o estÃ¡ executando para evitar
     * mensagens de erros que parecam-se com bug. O programa nao executa como usuÃ¡rio mesmo sem essa validacao.*/
    if (getuid()){
    ...
    if (FILE *f = fopen(INIFILE,"r")){
        ...
        /*Se nao houver definicao do arquivo ini, utiliza os proprios valores definidos no array, no inÃ­cio das declaracoes do codigo.*/
        frequencies[0]       = pt.get<float>("frequency.radio0", frequencies[0]); //!Frequencia do rÃ¡dio 0 configurado no arquivo /home/pi/AFMultiRadio.ini
        frequencies[1]       = pt.get<float>("frequency.radio1", frequencies[1]); //!Frequencia do rÃ¡dio 1 configurado no arquivo /home/pi/AFMultiRadio.ini
        frequencies[2]       = pt.get<float>("frequency.radio2", frequencies[2]); //!Frequencia do rÃ¡dio 2 configurado no arquivo /home/pi/AFMultiRadio.ini
        ...
        services.listener[0] = pt.get<bool>("listener.radio0", true); //!Escuta por clients no rÃ¡dio 0, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.listener[1] = pt.get<bool>("listener.radio1", true); //!Escuta por clients no rÃ¡dio 1, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.listener[2] = pt.get<bool>("listener.radio2", true); //!Escuta por clients no rÃ¡dio 2, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        
        services.sender[0]   = pt.get<bool>("sender.radio0",true); //!Envio de firmware pelo rÃ¡dio 0, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.sender[1]   = pt.get<bool>("sender.radio1",false); //!Envio de firmware pelo rÃ¡dio 1, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.sender[2]   = pt.get<bool>("sender.radio2",false); //!Envio de firmware pelo rÃ¡dio 2, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        
        services.bridge[0]   = pt.get<bool>("bridge.radio0", false); //!Transferencia entre concentradores, pelo rÃ¡dio 0, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.bridge[1]   = pt.get<bool>("bridge.radio1", false); //!Transferencia entre concentradores, pelo rÃ¡dio 1, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)
        services.bridge[2]   = pt.get<bool>("bridge.radio2", true); //!Transferencia entre concentradores, pelo rÃ¡dio 2, configurado no arquivo /home/pi/AFMultiRadio.ini (boolean)

        number_of_radios_to_initialize = pt.get<uint8_t>("start.initialize", 3); //Define o nÃºmero de rÃ¡dios que tem a placa. Os rÃ¡dios serÃ£o inicializados na ordem u2, u3 e u7. Na ausÃªncia do parÃ¢metro, os 3.
        ...
        /*! Por padrao, o log e desabilitado, mas se for modificada a variÃ¡vel logger,
        jÃ¡ estÃ¡ garantido um nome de diretorio para evitar bugs. */
    ...
    char *fvalue = NULL; //!Armazena o argumento para o parametro -f (frequencias dos rÃ¡dios).
  ...
  for (int i=0;i<number_of_radios_to_initialize;i++){
    ...
    while (!force_exit) {
        ...
        if (rf95[radioNumber].available()) {
          ...
          if (rf95[radioNumber].recv(buf, &len)) {
              if (buf[BUF_START] == '$' && buf[BUF_END] == '\n'){
                ...
                debug("Mensagem invalida - rÃ¡dio ");
                debug(to_string(radioNumber));
                debug(" Addr: ");
                debug(to_string(from));
                debug(" Buffer: ");
        ...
        /*
         * Se nao for definido no arquivo ini um ou mais rÃ¡dios para envio de firmware, essa condicional nao e executada nunca. Definicao:
         * [sender]
         * radio0=true
         * radio1=false
         * radio2=false
