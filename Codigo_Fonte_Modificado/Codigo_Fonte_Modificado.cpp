    ...
    const std::string SERVER_ADDRESS { "tcp://34.234.193.23:1883" };
    const std::string CLIENT_ID { "rasp" };

    void publish_message(mqtt::async_client& client, const std::string& topic, const std::string& message) {
    ...
    * */
    ...
    //exemplo de mensagem, comecando em $ e terminando com \n. Os Bytes internos representam a mensagem
    ...
    /***********************************************/
    #define MESSAGE_LENGTH      20               //6 Bytes used in the message ((0)$ (1)sensorA (2)sensorB (3)sensorC (4)sensorD (5)\n)
    ...
    while (!force_exit) {
        ...
        if (rf95[radioNumber].available()) {
            ...
            if (rf95[radioNumber].recv(buf, &len)) {
                ...
                for (int i = BUF_START; i < len; i++) { // Replace MESSAGE_LENGTH with len
                    debug(to_string(buf[i]) + " ");
                    transformed_message += static_cast<char>(buf[i]); // Convert the integer to its corresponding character
                }

                debug("Transformed message: ");
                debug(transformed_message);
                debug("\n");

                std::string topic = "rasp/test/";

                publish_message(client, topic, transformed_message);
